...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    ownerPerformance: owners.map((owner) => ({ id: owner.id, name: owner.name, tenants: owner._count.tenants, records: owner._count.administeredBills + owner._count.administeredRent, pending: bills.filter((item) => item.adminId === owner.id && item.adminVerificationStatus === "PENDING").length + rents.filter((item) => item.adminId === owner.id && item.adminVerificationStatus === "PENDING").length })),
