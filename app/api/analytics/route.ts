...
export async function GET() {
  ...
  const [rents, bills, documents, properties, adminCount, tenantCount, supportOpen] = await Promise.all([
    prisma.rentRecord.findMany({ where: scope, select: { billingMonth: true, billingYear: true, amount: true, adminVerificationStatus: true, adminId: true } }),
    ...
    user.role === "MASTER_ADMIN" ? prisma.user.count({ where: { role: "ADMIN", status: "APPROVED" } }) : Promise.resolve(user.role === "ADMIN" ? 1 : 0),
    prisma.user.count({ where: user.role === "MASTER_ADMIN" ? { role: "TENANT", status: "APPROVED" } : user.role === "ADMIN" ? { role: "TENANT", adminId: user.id, status: "APPROVED" } : { id: user.id } }),
    user.role === "MASTER_ADMIN" ? prisma.supportRequest.count({ where: { status: "OPEN" } }) : Promise.resolve(0)
  ]);
  const owners = user.role === "MASTER_ADMIN" ? await prisma.user.findMany({ where: { role: "ADMIN", status: "APPROVED" }, select: { id: true, name: true, email: true, _count: { select: { tenants: true, administeredBills: true, administeredRent: true } } }, orderBy: { name: "asc" } }) : [];
  ...
  return NextResponse.json({
    stats: {
      ...
      tenantMarkedPaid: bills.filter((item) => item.tenantPaymentStatus === "TENANT_MARKED_PAID").length,
      verifiedPaid: countBill("VERIFIED_PAID"), pendingVerification: countBill("PENDING") + countRent("PENDING"), rejectedClaims: countBill("REJECTED_CLAIM") + countRent("REJECTED_CLAIM"),
      agreements: countDocument("AGREEMENT"), idProofs: countDocument("ID_PROOF"), verifiedDocuments: documents.filter((item) => item.status === "VERIFIED").length, pendingDocuments: documents.filter((item) => item.status === "PENDING").length,
