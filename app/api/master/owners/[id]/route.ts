...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const owner = await prisma.user.findFirst({ where: { id, role: "ADMIN", isDeleted: false }, select: { id: true, name: true } });
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  await logDeletion(user, { id, type: "OWNER", name: owner.name }, parsed.data.reason);
