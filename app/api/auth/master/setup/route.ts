...
export async function POST(request: Request) {
  ...
  if (!masterCreationEnabled() || !allowedEmail) {
    await logActivity({ action: "MASTER_SETUP_FAILED", targetType: "AUTHENTICATION", description: "A disabled Master Admin setup attempt was rejected." });
    return NextResponse.json({ error: "Master Admin setup is unavailable." }, { status: 403 });
  }
  if (await prisma.user.count({ where: { role: "MASTER_ADMIN", isDeleted: false } })) {
  ...
  if (email !== allowedEmail) {
    await logActivity({ action: "MASTER_SETUP_FAILED", targetType: "AUTHENTICATION", description: "A Master Admin setup attempt used an email outside the allowlist." });
    return NextResponse.json({ error: "Master Admin setup is unavailable." }, { status: 403 });
  }
  ...
  await logActivity({ actorId: user.id, actorRole: user.role, actorName: user.name, action: "MASTER_ADMIN_CREATED", targetId: user.id, targetType: "USER", targetName: user.name, description: "The first Master Admin account was created through protected setup." });
