...
type View = "overview" | "analytics" | "usage" | "owners" | "master-tenants" | "support" | "activity" | "rent" | "bills" | "documents" | "complaints" | "notices" | "receipts" | "calendar" | "tenants";
type Bill = {
...
function AuthModal({ mode, setMode, busy, onSubmit, onNotify }: { mode: "login" | "signup" | "forgot" | "support" | "help"; setMode: (mode: "login" | "signup" | "forgot" | "support" | "help" | null) => void; busy: boolean; onSubmit: (payload: Record<string, unknown>, mode: "login" | "signup") => void; onNotify: (type: "success" | "error", text: string) => void }) {
  ...
  useEffect(() => { if (mode === "signup") request<{ owners: typeof owners }>("/api/owners").then((data) => setOwners(data.owners)).catch(() => setOwners([])); request<{ setupAvailable: boolean }>("/api/auth/master/setup").then((data) => setSetupAvailable(data.setupAvailable)).catch(() => setSetupAvailable(false)); }, [mode]);
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true"><section className="auth-modal"><button className="icon-button close" aria-label="Close" onClick={() => setMode(null)}><X /></button><div className="auth-brand"><span>RW</span><div><p className="kicker">Secure rental portal</p><h2>{title}</h2></div></div>
    ...
    {(mode === "login" || mode === "signup") && <a className="master-portal-link" href="/master-admin"><ShieldCheck />Master Admin Secure Access<ArrowRight /></a>}
    {mode === "signup" && setupAvailable && <a className="master-portal-link setup" href="/master-admin"><KeyRound />Secure Master Admin Setup<ArrowRight /></a>}
