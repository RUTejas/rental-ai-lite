...
type View = "overview" | "analytics" | "owners" | "master-tenants" | "support" | "activity" | "rent" | "bills" | "documents" | "tenants";
type Bill = {
...
export function RentWiseApp() {
  ...
  async function authenticate(payload: Record<string, unknown>, mode: "login" | "signup") {
    setBusy(true);
    try {
      const data = await request<{ user?: SessionUser; pending?: boolean; message?: string }>(`/api/auth/${mode}`, { method: "POST", body: JSON.stringify(payload) });
...
function Landing({ onAuth }: { onAuth: (mode: "login" | "signup" | "forgot" | "support" | "help") => void }) {
  ...
  return <main className="landing">
    ...
    <section className="role-section" id="roles"><p className="kicker">One system · Three perspectives</p><h2>Everyone sees exactly what they need.</h2><div className="role-grid">
      <RoleCard icon={ShieldCheck} label="Master Admin" text="Portfolio-wide oversight, owner summaries, and all utility records." onClick={() => window.location.assign("/master-admin")} />
...
function AuthModal({ mode, setMode, busy, onSubmit, onNotify }: { mode: "login" | "signup" | "forgot" | "support" | "help"; setMode: (mode: "login" | "signup" | "forgot" | "support" | "help" | null) => void; busy: boolean; onSubmit: (payload: Record<string, unknown>, mode: "login" | "signup") => void; onNotify: (type: "success" | "error", text: string) => void }) {
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true"><section className="auth-modal"><button className="icon-button close" aria-label="Close" onClick={() => setMode(null)}><X /></button><div className="auth-brand"><span>RW</span><div><p className="kicker">Secure rental portal</p><h2>{title}</h2></div></div>
    ...
    {mode === "login" && <a className="master-portal-link" href="/master-admin"><ShieldCheck />Master Admin Secure Access<ArrowRight /></a>}
...
function Dashboard({ user, bills, tenants, rents, documents, analytics, view, mobileNav, setMobileNav, setView, onLogout, onNotify, setBills, setTenants, setRents, setDocuments }: {
  ...
  const nav = [{ id: "overview" as View, label: user.role === "MASTER_ADMIN" ? "Command center" : "Overview", icon: LayoutDashboard }, ...(user.role !== "TENANT" ? [{ id: "analytics" as View, label: "Analytics", icon: BarChart3 }] : []), ...(user.role === "MASTER_ADMIN" ? [{ id: "owners" as View, label: "Owners", icon: Building2 }, { id: "master-tenants" as View, label: "All tenants", icon: UsersRound }, { id: "support" as View, label: "Issue reports", icon: LifeBuoy }, { id: "activity" as View, label: "Activity logs", icon: Activity }] : []), { id: "rent" as View, label: user.role === "TENANT" ? "My rent" : "Rent tracking", icon: WalletCards }, { id: "bills" as View, label: user.role === "TENANT" ? "My bills" : user.role === "MASTER_ADMIN" ? "All utility bills" : "Utility bills", icon: ReceiptText }, { id: "documents" as View, label: user.role === "TENANT" ? "My documents" : "Documents", icon: FileText }, ...(user.role === "ADMIN" ? [{ id: "tenants" as View, label: "Tenants", icon: UsersRound }] : [])];
  ...
  return <div className="dashboard-shell">
    ...
    <main className="dashboard-main"><header className="dash-topbar"><button className="icon-button menu-button" onClick={() => setMobileNav(true)} aria-label="Open menu"><Menu /></button><div><p>{roleName(user.role)} portal</p><h1>{viewTitle(view, user.role)}</h1></div><div className="top-actions"><button className="icon-button notification" aria-label="Notifications" onClick={() => onNotify("success", "You’re all caught up—no new notifications.")}><Bell /><span /></button><Avatar name={user.name} /></div></header>
      ...
      {user.role === "MASTER_ADMIN" && (view === "owners" || view === "master-tenants" || view === "support" || view === "activity") && <MasterAdminConsole mode={view} analytics={analytics} notify={onNotify} />}
...
function viewTitle(view: View, role: Role) { if (view === "owners") return "Owner governance"; if (view === "master-tenants") return "Tenant visibility"; if (view === "support") return "Issue management"; if (view === "activity") return "Security audit"; if (view === "tenants") return "Tenant management"; if (view === "analytics") return role === "MASTER_ADMIN" ? "Master analytics" : "Owner analytics"; if (view === "rent") return role === "TENANT" ? "My rent" : "Rent tracking"; if (view === "documents") return role === "TENANT" ? "My documents" : "Document verification"; if (view === "bills") return role === "TENANT" ? "My bills" : "Utility records"; return role === "MASTER_ADMIN" ? "Command center" : role === "ADMIN" ? "Owner dashboard" : "My rental dashboard"; }
